...
Status: in progress. The first slice adds an explicit, audited Stripe customer link to each existing-client app and an immediate, server-side subscription refresh. The client workspace displays the current and historical subscription status, product and price, billing interval, period end, trial and cancellation state, and latest invoice status without exposing Stripe credentials or allowing subscription mutations. Migration `028_client_stripe_subscriptions.sql` was applied on 2026-08-23. The second slice refreshes linked clients immediately from verified Stripe invoice and subscription events and adds an atomic, bounded six-hour reconciliation fallback to the existing payment-recovery cron. Failed refreshes retain sanitized status on the client while successful refreshes replace the snapshot transactionally. Migration `029_client_subscription_reconciliation.sql` was applied and verified on 2026-08-23. The third slice adds reusable, version-pinned email and Slack playbooks with Stripe-status eligibility, manual draft preparation from each client, editing and explicit approval or cancellation, audit history, export coverage, and retention previews. Approval records a decision but deliberately cannot send a provider message; delivery remains a separate future slice with another safety checkpoint. Migration `030_client_success_playbook_drafts.sql` was applied on 2026-08-23. The fourth slice makes manual CRM relationship state authoritative over Stripe, adds EpsiFlow Direct and Stripe-plan segmentation, active/churned/closed states, an explicit client-success switch, churn visibility, audit history, and a full stored-email context envelope for the future drafting agent. Slack history sync and Wise balance tracking remain deferred. Migration `031_client_relationship_state.sql` was applied on 2026-08-23. The fifth slice adds disabled-by-default scheduled check-ins, Stripe-cancellation follow-ups, and churn-reactivation drafts to the weekend-safe worker. Trigger windows are atomic and idempotent, CRM state is rechecked, synchronized-email context counts are pinned, and every result remains an approval-only template draft. Migration `032_scheduled_client_playbook_drafts.sql` was applied successfully on 2026-08-23. The sixth slice adds a disabled-by-default, atomic context-aware drafting queue on the same Vercel worker. It reads every synchronized client email plus CRM and subscription facts, treats customer content as untrusted, refuses silent truncation, uses strict structured output, validates source citations, displays agent warnings and provenance in Approvals, and still cannot send email or Slack. Migration `033_context_aware_client_drafting_agent.sql` was applied successfully on 2026-08-23. Model requests route through Vercel AI Gateway to `openai/gpt-5.6-luna` with medium reasoning; production uses Vercel OIDC authentication and local development can use an AI Gateway API key. The seventh slice adds four draft presets for monthly client health, EpsiFlow Direct payment checks, Stripe-plan recovery, and new-lead education. Their sales-recommended audiences and cadences, fallback templates, and exact AI instructions are visible in accessible detail modals and editable through immutable versions. The lead agent uses every stored incoming reply and sent campaign message. Migration `034_epsiflow_playbook_library.sql` was applied successfully on 2026-08-24. Migration `035_lead_relationship_playbooks.sql` adds a first-class Lead relationship type, carries the existing lead preset activation into an approval-gated manual playbook, and makes relationship saves visibly reflect the confirmed server value. Migration `036_persistent_client_playbook_assignments.sql` is ready to apply; it persists one assignment per client, monitors unanswered inbound messages plus follow-up and periodic-review windows, queues context-aware drafts, strengthens the lead onboarding/pricing prompt, and preserves the no-delivery approval boundary. Migration `037_authoritative_epsiflow_pricing.sql` is ready to apply after 036; it creates a new immutable lead-playbook version containing the confirmed Stripe tier table and EpsiFlow Direct charges. Migration `038_client_agent_retry.sql` adds an audited retry for terminal AI-draft failures; the Vercel workers now forward the runtime OIDC request header to AI Gateway. Migration `039_ai_draft_feedback_regeneration.sql` adds operator feedback and repeated AI regeneration for any open client draft while preserving approval-only delivery. Migration `040_immediate_client_agent_generation.sql` scopes generation to one authenticated client draft and makes the regeneration button invoke the engine immediately, with batch cron fallback.
