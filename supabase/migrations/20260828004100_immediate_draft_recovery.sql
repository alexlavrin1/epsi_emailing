...
-- Reinstall the exact-draft claim so this migration also repairs deployments
-- where migration 040 was skipped.
...
-- An authenticated operator may restart an unclaimed pending draft. A draft
-- already claimed by a worker remains locked and cannot be raced or replaced.
...
BEGIN
 ...
 IF target.status<>'draft' OR target.agent_status='processing' OR (target.agent_status='pending' AND target.agent_claimed_at IS NOT NULL) THEN RAISE EXCEPTION 'Draft cannot be regenerated while processing or after a decision'; END IF;
