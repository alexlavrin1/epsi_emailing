...
export async function getClientPlaybooks(supabase: SupabaseClient, organizationId: string) {
  ...
  const { data, error } = await supabase.from("client_playbooks")
    .select("id,name,description,channel,trigger_type,eligible_subscription_statuses,eligible_client_segments,eligible_relationship_states,cooldown_days,status,current_version,updated_at,versions:client_playbook_versions(version,subject_template,body_template)")
  ...
  const playbooks = (data ?? []).map(row => {
    ...
    return { id: row.id, name: row.name, description: row.description, channel: row.channel as ClientPlaybook["channel"], triggerType:row.trigger_type, eligibleStatuses: row.eligible_subscription_statuses || [], eligibleSegments:row.eligible_client_segments||[], eligibleRelationships:row.eligible_relationship_states||[], cooldownDays:row.cooldown_days, status: row.status as ClientPlaybook["status"], currentVersion: row.current_version, subjectTemplate: current?.subject_template ?? null, bodyTemplate: current?.body_template || "", versions: versions.length, updatedAt: row.updated_at };
  });
...
export async function getActiveClientPlaybooks(supabase: SupabaseClient, organizationId: string) {
  ...
  return { ready: result.ready, playbooks: result.playbooks.filter(playbook => playbook.status === "active" && playbook.triggerType === "manual_client_checkin") };
}
