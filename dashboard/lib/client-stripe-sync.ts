...
export async function triggerClientStripeSync(supabase: SupabaseClient, clientAppId: string) {
  ...
  try {
    const response = await fetch(new URL("/api/client-stripe-sync", engineUrl), {
      method: "POST",
