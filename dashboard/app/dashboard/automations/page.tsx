...
import { Activity, AlertTriangle, ArrowRight, CheckCheck, CircleStop, Clock3, Gauge, GitBranch, History, MailCheck, PauseCircle, PlayCircle, ShieldCheck, Workflow } from "lucide-react";
...
export default async function AutomationsPage() {
  ...
  return <main className="dashboard-main" id="main-content">
    ...
    {data.limits.ready ? <section className="automation-rate-limits" aria-labelledby="rate-limit-heading"><header><span className="rate-limit-icon" aria-hidden="true"><Gauge size={20} /></span><div><p className="eyebrow">Volume guardrail</p><h2 id="rate-limit-heading">Hourly automation capacity</h2><p>New runs stop at the organization limit and resume automatically as the rolling one-hour window advances.</p></div></header><div className="rate-limit-usage"><div><span>Runs used this hour</span><strong>{data.limits.usedThisHour.toLocaleString()} <small>/ {data.limits.hourlyLimit.toLocaleString()}</small></strong></div><progress value={Math.min(data.limits.usedThisHour, data.limits.hourlyLimit)} max={data.limits.hourlyLimit} aria-label={`${data.limits.usedThisHour} of ${data.limits.hourlyLimit} automation runs used this hour`} /><small>{data.limits.rateLimited24h.toLocaleString()} trigger{data.limits.rateLimited24h === 1 ? "" : "s"} blocked in the last 24 hours · blocked triggers are recorded in the audit log</small></div><AutomationRateLimitControl hourlyLimit={data.limits.hourlyLimit} isAdmin={isAdmin} /></section> : null}
