...
function canEditClientDraft(draft: ClientDraft) {
  return draft.status === "draft" && (!['pending','processing'].includes(draft.agentStatus) || (draft.agentStatus === "pending" && Boolean(draft.agentFailureCode)));
}
...
export default async function ApprovalsPage() {
  ...
  return <main className="dashboard-main" id="main-content">
    ...
    {!data.clientDraftsReady ? <section className="panel setup-panel"><MessageSquareText size={20} aria-hidden="true" /><div><strong>Context-aware client drafts are ready to install</strong><p>Apply migration 033 to review agent provenance and cited sources.</p></div></section> : null}
    ...
    <section className="approval-section" aria-labelledby="client-draft-approvals-heading"><div className="section-heading"><div><p className="eyebrow">Client success</p><h2 id="client-draft-approvals-heading">Playbook drafts</h2></div><span className="count-badge">{data.clientDrafts.length}</span></div>
      {data.clientDrafts.length ? <div className="approval-list">{data.clientDrafts.map(draft => {
        ...
        return <article className="approval-card client-playbook-draft-card" key={draft.id}><div className="approval-card-head"><span className="activity-icon">{draft.channel === "email" ? <Mail size={16} aria-hidden="true" /> : <MessageSquareText size={16} aria-hidden="true" />}</span><div><strong>{draft.contactName} · {draft.appName}</strong><small>{draft.channel} · {draft.recipient}</small><small className="automation-source"><Workflow size={12} aria-hidden="true" />{draft.playbookName} · v{draft.playbookVersion} · {draft.generationMode}</small><small>{draft.contextMessageCount} stored email messages available as context</small></div><span className={`status-badge status-${draft.status}`}>{draft.status}</span></div>{editable ? <ClientPlaybookDraftControl key={`${draft.id}:${draft.agentRegenerationCount}`} id={draft.id} clientAppId={draft.appId} channel={draft.channel} subject={draft.subject} body={draft.body} contact={draft.contactName} /> : <>{draft.subject ? <h3 className="client-draft-subject">{draft.subject}</h3> : null}<div className="approval-preview">{draft.body}</div></>}<DraftEvidence draft={draft}/><footer><time dateTime={draft.createdAt}>Created {formatWhen(draft.createdAt)}</time>{draft.status === "draft" && !editable ? <span className="campaign-locked">Awaiting context-aware draft</span> : draft.status !== "draft" ? <Link className="secondary-button compact-button" href={`/dashboard/clients/${draft.appId}`}>Open client</Link> : null}</footer></article>;
      })}</div> : <div className="empty-state approval-empty"><MessageSquareText size={22} aria-hidden="true" /><strong>No client playbook drafts</strong><p>Prepare one from an eligible client page. No playbook can send from this slice.</p></div>}
    </section>
