...
export default async function ApprovalsPage({ searchParams }: { searchParams: Promise<{ drafts?: string }> }) {
  ...
  return <main className="dashboard-main" id="main-content">
    ...
    <section className="approval-section" aria-labelledby="approval-inbox-heading"><div className="section-heading"><div><p className="eyebrow">Conversations</p><h2 id="approval-inbox-heading">Approval inbox</h2></div><nav className="performance-range draft-filter" aria-label="Filter approvals by delivery"><Link href="/dashboard/approvals" aria-current={draftsFilter === "all" ? "page" : undefined}>All</Link><Link href="/dashboard/approvals?drafts=sent" aria-current={draftsFilter === "sent" ? "page" : undefined}>Sent</Link><Link href="/dashboard/approvals?drafts=unsent" aria-current={draftsFilter === "unsent" ? "page" : undefined}>Not sent</Link></nav><span className="count-badge">{clientDrafts.length + replyDrafts.length}</span></div>
