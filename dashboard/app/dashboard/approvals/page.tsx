...
function DraftEvidence({ draft }: { draft: ClientDraft }) {
  return <aside className="client-draft-evidence" aria-label="Draft generation evidence">
    <div><strong>{draft.agentStatus === "completed" ? "AI-generated draft" : `Agent status: ${draft.agentStatus.replaceAll("_", " ")}`}</strong>{draft.agentModel ? <small>Model: {draft.agentModel}</small> : null}</div>
