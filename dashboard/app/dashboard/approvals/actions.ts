...
export async function decideClientPlaybookDraft(_state: ApprovalActionState, formData: FormData): Promise<ApprovalActionState> {
  ...
  if (error) {
    if (/generation is still in progress|preparation is still in progress/i.test(error.message)) return { ok: false, message: "AI generation is still pending. Generate or wait for the completed draft before approving it." };
    return { ok: false, message: /schema cache|Could not find|does not exist/i.test(error.message) ? "Email delivery requires migration 042." : "Unable to record this decision." };
  }
  revalidatePath("/dashboard/approvals"); revalidatePath("/dashboard/audit");
