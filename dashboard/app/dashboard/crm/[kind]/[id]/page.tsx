...
export default async function ContactDetailPage({ params }: { params: Promise<{ kind: string; id: string }> }) {
  ...
  const computedStage: LifecycleStage = contact.kind === "customer"
    ? contact.status === "suppressed" ? "suppressed" : contact.status === "recovery open" ? "at_risk" : "client"
    : ["bounced", "unsubscribed", "suppressed"].includes(contact.status) ? "suppressed" : contact.status === "replied" ? "interested" : "prospect";
  const effectiveStage = actionData.lifecycleStage ?? computedStage;
  return (
    <main className="dashboard-main" id="main-content">
      <Link className="back-row" href="/dashboard/crm"><ArrowLeft size={16} aria-hidden="true" />Back to CRM</Link>
      <header className="contact-header"><div className="contact-avatar"><UserRound size={28} aria-hidden="true" /></div><div><p className="eyebrow">{contact.kind === "customer" ? "Client" : "Prospect"}</p><h1>{contact.name}</h1><p>{contact.title} · {contact.company}</p></div><span className={`status-badge status-${effectiveStage.replace(/_/g, "-")}`}>{effectiveStage.replace(/_/g, " ")}</span></header>
