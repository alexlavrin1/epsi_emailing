...
import { ArrowLeft, ArrowUpRight, Clock3, CreditCard, Globe2, Mail, MessageSquareText, UserRound, Users } from "lucide-react";
import { requireMembership } from "../../../../lib/auth";
import { createSupabaseServerClient } from "../../../../lib/supabase-server";
import { formatWhen } from "../../../../lib/dashboard-data";
import { getClientAppDetail } from "../../../../lib/client-data";
import { ClientContactForm, ClientSlackAssignment, ClientSlackConnectLink, ClientStripeLink } from "../../../components/client-forms";
...
function money(amount: number | null, currency: string | null) {
  ...
  catch { return `${amount / 100} ${currency.toUpperCase()}`; }
}
...
export default async function ClientAppPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const data = supabase ? await getClientAppDetail(supabase, membership.organization.id, id) : { ready: false, app: null, messages: [], stripe: null };
  ...
  return <main className="dashboard-main" id="main-content">
    ...
    <div className="client-detail-layout">
      ...
      <aside className="client-side-column" aria-label="Client contacts">
        <section className="panel client-stripe-panel"><div className="panel-heading"><div><p className="eyebrow">Stripe</p><h2>Subscription</h2></div><CreditCard size={18} aria-hidden="true" /></div>
          {!data.stripe?.ready ? <div className="setup-inline"><strong>Stripe subscriptions are ready to install</strong><p>Apply migration 028, then link this client’s Stripe customer.</p></div> : <>
            {data.stripe.customerId ? <div className="client-stripe-identity"><strong>{data.stripe.customerName || data.stripe.customerEmail || data.stripe.customerId}</strong><code>{data.stripe.customerId}</code><span className={`status-badge status-${data.stripe.syncStatus}`}>{data.stripe.syncStatus}</span>{data.stripe.lastSyncAt ? <small>Checked {formatWhen(data.stripe.lastSyncAt)}</small> : null}{data.stripe.failureCode ? <small className="action-feedback error">Sync failed · {data.stripe.failureCode}</small> : null}</div> : <p className="muted">Link the Stripe customer to see current and past subscriptions.</p>}
            {data.stripe.subscriptions.length ? <div className="client-subscription-list">{data.stripe.subscriptions.map(subscription => <article key={subscription.id}><header><strong>{subscription.productName || subscription.priceNickname || "Stripe subscription"}</strong><span className={`status-badge status-${subscription.status}`}>{subscription.status.replaceAll("_", " ")}</span></header><code>{subscription.stripeSubscriptionId}</code><dl>{money(subscription.unitAmount, subscription.currency) ? <><dt>Price</dt><dd>{money(subscription.unitAmount, subscription.currency)}{subscription.billingInterval ? ` / ${subscription.intervalCount && subscription.intervalCount > 1 ? `${subscription.intervalCount} ` : ""}${subscription.billingInterval}` : ""}</dd></> : null}{subscription.quantity !== null ? <><dt>Quantity</dt><dd>{subscription.quantity}</dd></> : null}{subscription.currentPeriodEnd ? <><dt>Current period</dt><dd>Ends {formatWhen(subscription.currentPeriodEnd)}</dd></> : null}{subscription.trialEnd ? <><dt>Trial</dt><dd>Ends {formatWhen(subscription.trialEnd)}</dd></> : null}{subscription.cancelAtPeriodEnd ? <><dt>Cancellation</dt><dd>At period end</dd></> : null}{subscription.latestInvoiceStatus ? <><dt>Latest invoice</dt><dd>{subscription.latestInvoiceStatus}</dd></> : null}</dl></article>)}</div> : data.stripe.syncStatus === "synced" ? <div className="empty-state compact-empty"><strong>No subscriptions</strong><p>The linked Stripe customer has no subscriptions.</p></div> : null}
            <ClientStripeLink clientAppId={app.id} currentCustomerId={data.stripe.customerId} />
          </>}
        </section>
        <section className="panel"><div className="panel-heading"><div><p className="eyebrow">People</p><h2>Contacts</h2></div><Users size={18} aria-hidden="true" /></div><div className="client-contact-list">{app.contacts.map(contact => { const assignedUrl = contact.slackStatus === "assigned" && contact.slackTeamId && contact.slackChannelId ? `https://app.slack.com/client/${encodeURIComponent(contact.slackTeamId)}/${encodeURIComponent(contact.slackChannelId)}` : null; const slackUrl = contact.slackStatus === "linked" ? contact.slackChatUrl : assignedUrl; return <article className="client-contact-card" key={contact.id}><header><span><UserRound size={17} aria-hidden="true" /></span><div><strong>{contact.name}</strong><a href={`mailto:${contact.email}`}>{contact.email}</a></div></header><div className="client-contact-sync"><Clock3 size={13} aria-hidden="true" /><span>{contact.lastEmailSyncAt ? `Mailbox checked ${formatWhen(contact.lastEmailSyncAt)}` : "Mailbox match pending"}</span></div>{slackUrl ? <a className="client-slack-link" href={slackUrl} target="_blank" rel="noreferrer"><MessageSquareText size={15} aria-hidden="true" />Open {contact.slackChatLabel || contact.slackDisplayName || contact.slackName || "Slack chat"}<ArrowUpRight size={14} aria-hidden="true" /></a> : <><span className={`client-slack-status status-${contact.slackStatus}`}><MessageSquareText size={14} aria-hidden="true" />{contact.slackStatus === "pending" ? "Slack assignment pending" : contact.slackStatus === "failed" ? `Assignment failed · ${contact.slackFailureCode || "retry available"}` : "No Slack chat assigned"}</span>{contact.slackStatus !== "pending" ? <ClientSlackAssignment clientAppId={app.id} contactId={contact.id} slackName={contact.slackName} status={contact.slackStatus} /> : null}<ClientSlackConnectLink clientAppId={app.id} contactId={contact.id} /></>}</article>; })}</div></section>
