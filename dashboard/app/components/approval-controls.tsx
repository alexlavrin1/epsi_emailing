...
import { decideClientPlaybookDraft, disposeReply, queueReply, regenerateClientPlaybookAgentDraft, regenerateReplyDraft, retryRecoveryMessage, updateClientPlaybookDraft, updateReplyDraft, type ApprovalActionState } from "../dashboard/approvals/actions";
...
export function ReplyApprovalControl({ id, status, contact, body, canRegenerate = false }: { id: string; status: string; contact: string; body: string; canRegenerate?: boolean }) {
  ...
  const [regenerateState, regenerateAction] = useActionState(regenerateReplyDraft, initialState);
  ...
  return <div className="approval-control reply-draft-control"><div className="approval-primary-actions">{canRegenerate ? <form action={regenerateAction} onSubmit={event => { if (!window.confirm(`Regenerate this reply to ${contact}? The current version will remain unsent until its replacement is ready.`)) event.preventDefault(); }}><input type="hidden" name="reply_id" value={id} /><ActionButton label="Regenerate" pendingLabel="Queueing…" tone="secondary" /></form> : null}<form action={action} onSubmit={event => { if (!window.confirm(`Approve and queue this email reply to ${contact}? The backend will send it on the next outreach cycle.`)) event.preventDefault(); }}><input type="hidden" name="reply_id" value={id} /><ActionButton label={status === "failed" ? "Approve retry" : "Approve & queue"} pendingLabel="Queueing…" /></form><form action={cancelAction} onSubmit={event => { if (!window.confirm(`Cancel this reply to ${contact}? The draft will close and nothing will be sent.`)) event.preventDefault(); }}><input type="hidden" name="reply_id" value={id} /><input type="hidden" name="decision" value="cancel" /><ActionButton label="Cancel" pendingLabel="Cancelling…" tone="danger" /></form></div>{state.message ? <p className={state.ok ? "action-feedback success" : "action-feedback error"} role={state.ok ? "status" : "alert"}>{state.message}</p> : null}{regenerateState.message ? <p className={regenerateState.ok ? "action-feedback success" : "action-feedback error"} role={regenerateState.ok ? "status" : "alert"}>{regenerateState.message}</p> : null}<details className="approval-editor"><summary>Edit draft</summary><form className="action-form" action={editAction}><input type="hidden" name="reply_id" value={id} /><label>Reply text <span>(required)</span><textarea name="body" required minLength={1} maxLength={10000} defaultValue={body} /></label><ActionButton label="Save draft" pendingLabel="Saving…" tone="secondary" /></form>{editState.message ? <p className={editState.ok ? "action-feedback success" : "action-feedback error"} role={editState.ok ? "status" : "alert"}>{editState.message}</p> : null}</details><div className="approval-dispositions" aria-label="Skip this reply"><form action={skipAction} onSubmit={event => { if (!window.confirm(`Skip this reply to ${contact}? The draft will close and nothing will be sent.`)) event.preventDefault(); }}><input type="hidden" name="reply_id" value={id} /><input type="hidden" name="decision" value="skip" /><ActionButton label="Skip reply" pendingLabel="Skipping…" tone="secondary" /></form></div>{skipState.message ? <p className={skipState.ok ? "action-feedback success" : "action-feedback error"} role={skipState.ok ? "status" : "alert"}>{skipState.message}</p> : null}{cancelState.message ? <p className={cancelState.ok ? "action-feedback success" : "action-feedback error"} role={cancelState.ok ? "status" : "alert"}>{cancelState.message}</p> : null}</div>;
}
