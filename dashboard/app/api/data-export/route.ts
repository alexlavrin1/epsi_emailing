...
export async function POST(request: NextRequest) {
  try {
    ...
    const queries = {
      ...
      clientApps: client.from("client_apps").select("id,name,website_url,status,stripe_customer_id,stripe_customer_email,stripe_customer_name,stripe_sync_status,last_stripe_sync_at,created_at,updated_at").eq("organization_id", organization.id).order("created_at").limit(limit + 1),
