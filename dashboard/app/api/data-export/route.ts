...
export async function POST(request: NextRequest) {
  try {
    ...
    const queries = {
      ...
      clientPlaybookDrafts: client.from("client_playbook_drafts").select("id,playbook_id,playbook_version,client_app_id,client_contact_id,client_subscription_id,channel,recipient_label,subject,body,status,delivery_status,delivery_attempt_count,delivery_queued_at,delivery_claimed_at,delivered_at,delivery_failure_code,provider_message_id,generation_mode,context_message_count,context_latest_message_at,agent_status,agent_attempt_count,agent_generated_at,agent_model,agent_response_id,agent_failure_code,agent_context_sha256,agent_context_warnings,agent_regeneration_feedback,agent_regeneration_count,agent_regeneration_requested_by_user_id,agent_regeneration_requested_at,decided_at,created_at,updated_at").eq("organization_id", organization.id).order("created_at").limit(limit + 1),
