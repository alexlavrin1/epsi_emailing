...
async function getClientConversationContext(clientAppId) {
  const { data: app, error: appError } = await database.supabase.from('client_apps')
  ...
  for (let from = 0; ; from += 500) {
    const { data, error } = await database.supabase.from('client_email_messages')
