...
const { createStructuredClientDraft } = require('../src/integrations/openai/client');
...
test('context-aware drafting is disabled by default', async () => {
  const original = config.openai.clientSuccessAgentEnabled;
  try {
    config.openai.clientSuccessAgentEnabled = false;
  ...
  } finally { config.openai.clientSuccessAgentEnabled = original; }
});

test('generates an approval draft from complete context with validated citations', async () => {
  const original = config.openai.clientSuccessAgentEnabled; const completed = [];
  try {
    config.openai.clientSuccessAgentEnabled = true;
    const result = await generateClientSuccessAgentDrafts({
      ...
      openai: { createStructuredClientDraft: async prompts => {
...
test('fails closed when complete context exceeds the configured limit', async () => {
  const originalEnabled = config.openai.clientSuccessAgentEnabled; const originalLimit = config.openai.clientSuccessMaxContextChars; const failures = [];
  try {
    config.openai.clientSuccessAgentEnabled = true; config.openai.clientSuccessMaxContextChars = 20;
    const result = await generateClientSuccessAgentDrafts({ db: { claimClientPlaybookAgentDrafts: async () => [job], completeClientPlaybookAgentDraft: async () => assert.fail('must not complete'), failClientPlaybookAgentDraft: async (_id, code) => failures.push(code) }, getContext: async () => context, openai: { createStructuredClientDraft: async () => assert.fail('must not call model') } });
    assert.equal(result.failed, 1); assert.deepEqual(failures, ['client_context_too_large']);
  } finally { config.openai.clientSuccessAgentEnabled = originalEnabled; config.openai.clientSuccessMaxContextChars = originalLimit; }
});
...
test('uses the Responses API with strict structured output and provider storage disabled', async () => {
  let request;
  const result = await createStructuredClientDraft({ system: 'System', user: 'User' }, { apiKey: 'test-key', model: 'test-model', fetch: async (_url, options) => { request = JSON.parse(options.body); return { ok: true, status: 200, json: async () => ({ id: 'resp-1', status: 'completed', output: [{ type: 'message', content: [{ type: 'output_text', text: JSON.stringify({ subject: 'Subject', body: 'Body', source_message_ids: [], context_warnings: [] }) }] }] }) }; } });
